...
@section('content')
    ...
    <form action="{{ route('rotafazenda.index') }}" method="GET">
        ...
        <div class="d-flex gap-2 mb-3" name="barraDeBuscaEBotaoFiltro">
            ...
            <input name="barraDeBusca" type="text"
                class="form-control input-custom"placeholder="Pesquisar fazenda, setor ou talhão"
                value="{{ request('barraDeBusca') }}">

            <button type="button" name="botaoFiltro" class="btn btn-secondary" id="toggleFilters"
        ...
        <div id="advancedFilters" name="FiltrosAvancados" class="card p-3 mb-3" style="display: none;">
            <div class="row g-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" name="setor" class="form-control input-custom" placeholder="Buscar por setor"
                        value="{{ request('setor') }}">
                </div>

                <div class="col-md-3">
                    <label class="form-label">Variedade</label>
                    <input type="text" name="variedade" class="form-control input-custom"
                        placeholder="Buscar por variedade" value="{{ request('variedade') }}">
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="dataPlantio" class="form-control input-custom"
                        value="{{ request('dataPlantio') }}">
                </div>
            ...
            @forelse ($fazendas as $f)
                <tr>
                    ...
                    <td class="text-nowrap">
                        <a href="{{ route('rotafazenda.edit', $f->idFazenda) }}" class="btn btn-warning btn-sm"
                            name="botaoEditar"> <i class="bi bi-pencil"></i></a>

                        <form action="{{ route('rotafazenda.destroy', $f->idFazenda) }}" method="POST"
                            style="display:inline;">
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="page" value="{{ request('page') }}">
                            <button class="btn btn-danger btn-sm" name="botaoExcluir"
